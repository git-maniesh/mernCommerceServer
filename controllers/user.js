...
export const forgetpassword = asyncError(async (req, res, next) => {
  ...
  const randomNumber = Math.random() * (999999 - 10000) + 10000;
  ...
  await user.save();
  // console.log(otp);
  const message = `Your OTP for Reseting the password is ${otp} .\n Please ignore if you haven't requested. `;
  try {
    await sendEmail("OTP for Reseting the password ", user.email, message);
  } catch (error) {
    user.otp = null;
...
export const resetpassword = asyncError(async (req, res, next) => {
  ...
  if (!user)
    return next(new ErrorHandler("Incorrect  OTP or has been expireed", 400));
